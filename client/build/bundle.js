...
class Channel {
  ...
  onMessage(_event, payload, _ref, _var) {
    return payload;
  }
...
class LongPoll {
  ...
  timeout(method, arg1, contentType, body, timeout, ontimeout, arg6) {
...
async function connect(endpoint, token) {
  let socket2 = new Socket(endpoint, {
    params: { token }
  });
  socket2.connect({ params: { token } });
  return socket2;
}
async function join(socket2, channelName, token) {
  let channel3 = socket2.channel(channelName, { params: { token } });
  channel3.join();
  return channel3;
}
export {
